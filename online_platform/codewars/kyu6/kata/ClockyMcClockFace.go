// https://www.codewars.com/kata/59752e1f064d1261cb0000ec/train/go
...
func WhatTimeIsIt(angle int) string {
	...
	numMinutes := angle * 2
	hh := numMinutes / numMinutesPerHour
	if hh == 0 {
		hh = 12
	}
	mm := numMinutes % numMinutesPerHour
	return fmt.Sprintf("%02d:%02d", hh, mm)
}

func WaWhatTimeIsIt(angle int) string {
	// your code
	full := 360
	angle %= full
	if angle == 0 {
	...
	numMinutes := angle * 2
	return fmt.Sprintf("%02d:%02d", numMinutes/numMinutesPerHour, numMinutes%numMinutesPerHour)
}
